...
public class SQLiteAxolotlStore implements SignalProtocolStore {
	...
	/**
	 * The identities table is keyed by (account, name, fingerprint), not by per-device
	 * SignalProtocolAddress, so an exact single-device answer isn't always available.
	 * Only return a key when it's unambiguous; otherwise report unknown rather than guess.
	 */
	@Override
